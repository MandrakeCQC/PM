...
int main(int argc, char **argv)
{
   ...
   bool use_write = argv[4][0] == 'w';
   //bool use_ram = argv[3][0] == 'r';

   const uint64_t total_size = atof(argv[1]);
   const uint64_t thread_count = atof(argv[2]);
   int use_ram = thread_count / 2;    // thread_count = NVM_threads + DRAM_threads ; NVM_threads = DRAM_threads, share the same cores
   ...
   for (int t = 0; t<thread_count; t++) { // Spawn threads
      workers[t] = make_unique<thread>([&, t]() {
         ...
         //CPU_SET(((int)(thread_id/2))%num_cores, &cpuset);
         CPU_SET( ((thread_id / 2) + (thread_id % 2) * (num_cores / 2)) % num_cores, &cpuset);
	 ...
	 if (use_streaming) { 
         ...
         if (thread_id % 2 == 0) {   // use dram  0, 2, 4, ... use DRAM, 1,3,5,.. use NVM
	    { 
