...
print 'type device req_size threads ratio_dram fio_avg_lat(us) fio_50_lat(us) fio_p99_lat(us) fio_bw(MB/s) min_lat median_lat avg_lat p99_lat max_lat'
    ...
    for each_file in os.listdir(check_dir + '/' + subdir):
       if each_file == 'config.json':
           with open(check_dir + '/' + subdir + '/' + each_file, 'r')  as config_file:
               ...
               res1 = dict_from_file['type_pattern'] + ' ' + dict_from_file['device'] + ' ' + str(dict_from_file['req_size']) + ' ' + str(dict_from_file['cores']) +  ' '
               ...
               if 'write' not in dict_from_file['type_pattern']:
                   jump = False
