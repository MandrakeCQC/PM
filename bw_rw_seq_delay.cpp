...
int main(int argc, char **argv)
{
   ...
   if (argc < 5) {
      cout << "usage: " << argv[0] << " datasize(bytes) thread_count (nvm|ram) block_size(bytes) w read_threads read_delay write_delay" << endl;
      return 1;
   }
   ...
   int DRAM_readthreads = 0;
   int 
   srand(time(0));
   
   int NVM_read_delay = (int)atoi(argv[7])*2.3;
   int NVM_write_delay = (int)atoi(argv[8])*2.3;
   ...
   for (int t = 0; t<thread_count; t++) { // Spawn threads
      workers[t] = make_unique<thread>([&, t]() {
         ...
         for (uint64_t i = 0; i<iteration_count; i++) {
            random_offsets[i] = i * block_size;// + thread_id * block_size * iteration_count;
         }
/*{
std::lock_guard<std::mutex> iolock(iomutex);
cout<< "=== this is thread " << thread_id << "start= " << random_offsets[0] << endl;
}*/
         ...
         while (true) {
            for (uint64_t idx = 0; idx<iteration_count; idx++) {
               if (!running_flag) {
                ...
                if (last_time != 0) {
                    uint64_t curr = perf_counter();
                    uint64_t diff = curr - last_time;
                    if (thread_read && diff >= NVM_read_delay) {
                        last_time = curr;
                    }
                    else if (!thread_read && diff >= NVM_write_delay) {
                        last_time = curr;
                    }
                    else {
                        idx--;
               ...
               //tmp %= iteration_count;
               //uint64_t *start = (uint64_t *) (keys + random_offsets[tmp]);
