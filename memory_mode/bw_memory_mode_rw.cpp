...
int main(int argc, char **argv)
{
   ...
   const uint32_t write_ratio = atof(argv[6]);
   ...
   for (int t = 0; t<thread_count; t++) { // Spawn threads
	 ...
	 bool *random_rw = new bool[iteration_count];  // read false, write true
	    ...
	    random_rw[i] = false;
	    if (i < iteration_count / 100.0 * write_ratio)
	      random_rw[i] = true;
         }
         // Random
	 ...
	 start_barrier++;
	    ...
	    for (uint64_t idx = 0; idx<iteration_count; idx++) {
		  ...
		  assert(local_counter);
               ...
               if (idx == iteration_count / 100 * write_ratio && repeated_writes < 5) {
		 std::cout << "repeating writes" << std::endl;
		 idx = 0;
		 repeated_writes += 1;
	       }
               if (idx == iteration_count / 100 * write_ratio && repeated_writes == 5) {
