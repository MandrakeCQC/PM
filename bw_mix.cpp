...
int main(int argc, char **argv)
{
   ...
   for (int t = 0; t<thread_count; t++) { // Spawn threads
	 ...
	 if (use_streaming) { 
	    ...
	    cout << "=== this is thread " << thread_id  << " accessing dram, on CPU " << sched_getcpu() << endl;
	    }
	    ...
	    { 
	    std::lock_guard<std::mutex> iolock(iomutex);
	    cout << "=== this is thread " << thread_id  << " accessing NVM, on CPU " << sched_getcpu() << endl;
	    }
   ...
   double dram_gbs = ((dram_iterations * block_size) / (1000.0 * 1000.0 * 1000.0)) / (end - start);
   
   //double NVM_lat = (double)(global_lat_ns) / (global_iterations) ;
   //double DRAM_lat = (double)(dram_lat_ns) / (dram_iterations) ;
   double NVM_lat = (double)(global_lat_ns) / (global_iterations) / 2.3;
   double DRAM_lat = (double)(dram_lat_ns) / (dram_iterations) / 2.3;
   ...
   std::cout << "res use_clwb: " << use_clwb << " use_ram: " << use_ram << " use_streaming: " << use_streaming << " use_write: " << use_write << " thread_count: " << thread_count << " block_size: " << BLOCK_SIZE << " nvm: " << gbs << " " << NVM_lat << " dram: " << dram_gbs << " " << DRAM_lat << std::endl;
